...
$data['totalProducts'] = $this->totalCartQuantityCount();
    ...
    if($request->weight !== null && $request->weight !== 'undefined' && $request->weight !== '' ){
    

    $id = $request->weight;

    $weight = Order_Weight::find($id);

    $weight_amount = $weight->weight;
    $price = $weight->price;
...
//If cart's have same item but weight is different then we need to save that product as different product
...
// $file = view('frontend.invoice.invoice', compact($order))->render();
// Mail::to(auth()->user()->email)->send([], [], function ($message) use ($file) {
  
//    $message->attachData($file, 'invoice.html', ['mime' => 'text/html']);
// });
